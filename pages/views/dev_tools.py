...
def setupDB( request ):
  if Network.objects.count() == 0:
    ...
    unableOpen = Rejection( name = 'Unable to open file', subject = 'CFTS Request Rejected -- Unable to Open File', text = '''The file submitted for transfer is not a format we are unable to open, and therefore cannot be transferred to NIPR.  If there is a way you can convert this file to a format we are able to open such as .ppt, .xlsx, .doc or .pdf and the file contains proper classification markings we will transfer it.''' )
    unableOpen.save()
    ...
    redact = Rejection( name = 'redacted', subject = 'CFTS Transfer Request Rejected Redaction', text = '''Per DoD Regulation 5200 01 (pg 66 para (2); a redacted document requires a memo from the current Original Classification Authority (OCA) stating the document has been redacted and is authorized for the target network.''' )
    redact.save()
